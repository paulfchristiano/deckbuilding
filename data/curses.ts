...
export interface Curse {
    name: string
    events: CardSpec[]
    vpTargetMultiplier?: number  // defaults to 1 (no change)
}

const minorCursePool: Curse[] = []
const majorCursePool: Curse[] = []
...
function registerCursePair(minor: Curse, major: Curse): void {
    minorCursePool.push(minor)
    majorCursePool.push(major)
    for (const event of minor.events) registerSpec(event)
...
export function allMajorCurses(): Curse[] {
...
registerMirroredCurse((isMajor): Curse => {
    ...
    return {
        ...
        events: [{
            ...
            staticReplacers: [{
                kind: 'victory',
                text: [`The vp target is ${vpMultiplier}x higher.`],
                handles: () => false,
