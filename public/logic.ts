...
type Params = ResourceParams | CostParams | DrawParams | MoveParams
...
function gainPoints(n:number, source:Source=unk): Transform {
    return async function(state) {
        ...
        if (state.points >= 40) throw new Victory(state)
        return state
    }
}
...
interface Extras {
    ...
    replacers?:(c:Card) => TypedReplacer[];
    restriction?: Restriction;
    onBuy?:Effect;
}
function supplyForCard(card:CardSpec, cost:Cost, extra:Extras={}): CardSpec {
    return {name: card.name,
        fixedCost: cost,
        restriction: extra.restriction,
        effect: (supply:Card) => ({
            text: ((extra.onBuy === undefined) ? '' : extra.onBuy.text + ' ') + 
                `Create ${a(card.name)} in your discard pile.`,
            effect: doAll([
                create(card),
                (extra.onBuy === undefined) ? noop : extra.onBuy.effect
            ])
        }),
        relatedCards: [card],
...
function regroupEffect(n:number, source?:Source): Transform {
    return async function(state) {
...
function buyableFree(card:CardSpec, coins:number, test:'test'|null=null): void {
    return buyableAnd(card, coins, {onBuy: {text: '+1 buy.', effect:gainBuy}}, test)
}
...
const throneRoom:CardSpec = {name: 'Throne Room',
    ...
    effect: card => ({
        ...
        effect: payToDo(payDraw, async function(state) {
            let target; [state, target] = await choice(state,
                'Choose a card to play twice.',
                state.hand.map(asChoice))
            if (target == null) return state
...
const scavenger:CardSpec = {name: 'Scavenger',
    ...
    effect: card => ({
        ...
        effect: doAll([
            gainCoin(2),
            applyToTarget(c => move(c, 'hand'), 'Choose a card to put into your hand.', 'discard')
        ])
    })
}
buyable(scavenger, 4)
...
const hallOfMirrors:CardSpec = {name: 'Hall of Mirrors',
    ...
    triggers: card => [{
        ...
        kind:'afterPlay',
        handles: (e, state) => state.find(e.card).count('mirror') > 0 && e.source.name != card.name,
        effect: e => doAll([playAgain(e.card, card), removeToken(e.card, 'mirror')]),
    }]
}
...
const retrench:CardSpec = {name: 'Retrench',
    ...
    effect: card => ({
        ...
        effect: doAll([addToken(card, 'cost'), regroupEffect(5, card)])
    })
}
...
const repurpose:CardSpec = {name: 'Repurpose',
    ...
    effect: card => ({
        ...
        effect: async function(state) {
            return regroupEffect(state.draws, card)(state)
        }
    })
}
...
const oldSmith:CardSpec = {name: 'Old Smith',
    ...
    effect: card => ({
        ...
        effect: async function(state) {
            state = await draw(4 - state.find(card).charge)(state);
...
const shelter:CardSpec = {name: 'Shelter',
    effect: card => ({
        ...
        effect: async function(state) {
            let target:Card|null; [state, target] = await choice(state,
                'Choose a card to shelter.',
                state.play.map(asChoice))
            if (target != null) state = await addToken(target, 'shelter')(state)
            return state
        }
    })
}
    ...
    effect: card => ({
        ...
        effect: async function(state) {
            ...
            let target:Card|null; [state, target] = await choice(state,
                ...
                state.hand.map(asChoice))
            if (target != null) state = await doAll([target.play(card), trash(target), gainBuy])(state)
            return state
        }
    })
}
    ...
    effect: card => ({
        ...
        effect: async function(state) {
            let target; [state, target] = await choice(state,
                ...
                state.play.map(asChoice))
            if (target != null)  {
                state = await create(target.spec, 'play', 'end', c => addToken(c, 'echo'))(state)
            }
            return state
        }
    })
...
register(supplyForCard(echo, coin(4), {
    triggers: card => [{
        ...
        kind: 'move',
        handles: (x, state) => x.card.count('echo') > 0,
        effect: x => trash(x.card)
    }]
}))
...
const mastermind:CardSpec = {name: 'Mastermind',
    ...
    replacers: card => [{
        ...
        replace: x => ({...x, skip:true, effects:x.effects.concat([charge(card, 1)])})
    }],
    triggers: card => [{
...
const doubleTime:CardSpec = {name: 'Double Time',
    effect: cantripPlay,
    triggers: card => [{
        text: 'Whenever you pay @, that many charge tokens on this.',
        kind: 'cost',
        handles: () => true,
        effect: e => charge(card, e.cost.energy)
    }],
    replacers: card => [{
...
function applyToTarget(
    ...
    text:string,
    zone:ZoneName,
    filter:(c:Card, s:State) => boolean = () => true,
): Transform {
    return async function(state) {
        let target; [state, target] = await choice(state,
            text,
            state.getZone(zone).filter(c => filter(c, state)).map(asChoice))
...
const dragon:CardSpec = {name: 'Dragon',
    effect: card => ({
        text: 'Trash a card in your hand. +4 draw. +$4. +1 buy.',
...
const egg:CardSpec = {name: 'Egg',
    ...
    effect: card => ({
        ...
        effect: async function(state) {
            if (card.charge >= 2) return create(dragon, 'hand')(state)
...
const formation:CardSpec = {name: 'Formation',
    ...
    replacers: card => [{
        text: 'Cards in your hand cost @ less if you have a card with the same name'
         ...
         + ' Whenever this reduces a cost, discard this and +$2.',
        kind: 'cost',
        handles: (e, state) => (state.discard.concat(state.play).some(c => c.name == e.card.name)),
        replace: function(x:CostParams, state:State) {
            if (x.card.place == 'hand') {
                const newCost:Cost = subtractCost(x.cost, {energy:1})
                if (!eq(newCost, x.cost)) {
...
const traveler:CardSpec = {
    ...
    effect: card => ({
        ...
        effect: payToDo(payDraw, async function(state) {
            let target; [state, target] = await choice(state,
                `Choose a card to play with ${traveler.name}`,
                state.hand.filter(x => x.name != traveler.name).map(asChoice))
            ...
            if (n < 4) state = await charge(card, 1)(state)
            return state
        })
...
const industry:CardSpec = {
    ...
    effect: card => ({
        ...
        effect: async function(state) {
            return draw(Math.min(state.play.length, 5), card)(state)
        }
    })
}
buyable(industry, 4)
