...
var hotkeys = numHotkeys.concat(symbolHotkeys)
...
var IMap = /** @class */ (function () {
    ...
    IMap.prototype.get = function (x) {
        return this.f.get(x);
    };
    IMap.prototype.set = function (x, y) {
        if (this.inv.has(y))
            throw Error("Trying to assign duplicate values to IMap.");
        var oldy = this.f.get(x);
        if (oldy != undefined)
            this.inv.delete(oldy);
        this.f.set(x, y);
        this.inv.set(y, x);
    };
    IMap.prototype.has = function (x) {
        return this.f.has(x);
    };
    IMap.prototype.delete = function (x) {
        var y = this.f.get(x);
        if (y == undefined)
            return;
        this.f.delete(x);
    ...
    IMap.prototype.entries = function () {
...
var HotkeyMapper = /** @class */ (function () {
    ...
    HotkeyMapper.prototype.ensureCoverage = function (m, s, order) {
        var e_11, _a, e_12, _b;
        try {
            for (var _c = __values(m.keys()), _d = _c.next(); !_d.done; _d = _c.next()) {
                var id = _d.value;
                if (!s.has(id))
                    m.delete(id);
            }
    ...
    HotkeyMapper.prototype.update = function (state) {
        var supplyIds = new Set(state.supply.map(function (card) { return card.id; }));
        var handIds = new Set(state.hand.map(function (card) { return card.id; }));
        var playIds = new Set(state.play.map(function (card) { return card.id; }));
...
var youngSmith = { name: 'Young Smith',
    ...
    triggers: function (card) { return [{
            text: 'Whenever you gain time, you may remove up to that many charge tokens from this.' +
            ...
            effect: function (e) { return function (state) {
                return __awaiter(this, void 0, void 0, function () {
                    return __generator(this, function (_a) {
                        switch (_a.label) {
                            case 0:
                                if (!(state.find(e.before).place == 'discard')) return [3 /*break*/, 3];
                                return [4 /*yield*/, removeOneToken(e.before, 'mirror')(state)];
                            case 1:
                                state = _a.sent();
                                return [4 /*yield*/, e.before.play(card)(state)];
                            case 2:
                                state = _a.sent();
                                _a.label = 3;
                            case 3: return [2 /*return*/, state];
...
var expedite = { name: 'Expedite',
    ...
    effect: function (card) { return ({
        text: "The next time you create a card, if it's in your discard pile put it into your hand." +
...
var scavenger = { name: 'Scavenger',
    fixedCost: time(1),
    effect: function (card) { return ({
        text: '+$2. Put up to two cards from your discard pile on top of your deck.',
        effect: function (state) {
            return __awaiter(this, void 0, void 0, function () {
                var targets;
                var _a;
                return __generator(this, function (_b) {
                    switch (_b.label) {
                        ...
                        case 1:
                            state = _b.sent();
                            return [4 /*yield*/, multichoice(state, 'Choose up to two cards to put on top of your deck.', state.discard.map(asChoice), function (xs) { return xs.length <= 2; })];
                        case 2:
...
var woodcutter = { name: "Woodcutter",
    fixedCost: time(1),
    effect: function (card) { return ({ text: '+$3', effect: gainCoin(3) }); }
};
...
var publicWorks = { name: 'Public Works',
    effect: function (card) { return ({
        text: "The next card you buy this turn costs @ less.",
...
var composting = { name: 'Composting',
    triggers: function (card) { return [{
            text: 'Whenever you gain time, you may recycle that many cards from your discard pile.',
            kind: 'gainTime',
            handles: function (e) { return e.amount > 0; },
            effect: function (e) { return function (state) {
                return __awaiter(this, void 0, void 0, function () {
