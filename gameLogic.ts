...
export type Source = Card | Rule | 'act' | null

export function sourceHasName(s:Source, name:string): boolean {
    if (s == 'act' || s == null) return false
    return s.name == name
}
...
export type ZoneName = 'supply' | 'hand' | 'discard' | 'play' | 'events' | 'void' | 'potions' | 'relics'
...
export type CostResourceName = 'coin' | 'energy' | 'actions' | 'buys'
export const allCostResources:CostResourceName[] = ['coin', 'energy', 'actions', 'buys']
export type ResourceName = CostResourceName | 'points'
export const allResources:ResourceName[] = (allCostResources as ResourceName[]).concat(['points'])
...
export function serializeReplay(r:Replay): string {
    return r.actions.map(x => x.toString()).join(';')
}
            ...
            if (rule.triggers) {
                ...
                for (const rawTrigger of rule.triggers) {
                    if (rawTrigger.kind == e.kind) {
                        ...
                        if (trigger.handles(e, initialState, null)
                            && trigger.handles(e, state, null)) {
                            state = state.log(`Triggering ${rule.name} rule`)
                            state = await withTracking(
                                trigger.transform(e, state, null),
                                {kind:'trigger', trigger:trigger as Trigger<any>, card:ruleCard}
                            )(state)
                        }
                    }
                }
...
export function refreshEffect(n:number): Effect {
    return {
        text: [
...
export function fountainEffect(): Effect {
    return {
        text: [
            'Lose all $, actions, and buys.',
            `+5 actions, +1 buy.`
...
export const core:{cards:CardSpec[], events:CardSpec[]} = {
    cards: [],
    events:[]
}
...
export function buyTrigger(effect:Effect): Trigger<BuyEvent> {
    return {
        kind: 'buy',
...
export function afterBuyTrigger(effect:Effect): Trigger<AfterBuyEvent> {
    return {
        kind: 'afterBuy',
        handles: (e, s, c) => e.card.id == c!.id,
        transform: (e, s, c) => effect.transform(s, c!),
